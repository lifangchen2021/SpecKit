...
#                print(f"Computed Loss Threshold: {computed_loss_threshold}") 
    ...
    def plot_results(self, loss_history, x_values):
        ...
        self.ax2.clear()
        x_labels = self.A_header  
        
        self.ax2.step(x_labels, x_values, where='mid', color='orange')
        self.ax2.fill_between(x_labels, x_values, step='mid', color='orange', alpha=0.5)

        self.ax2.set_xlabel("Neutron Energy (MeV)")
        self.ax2.set_ylabel("Flux(/cm²)")
